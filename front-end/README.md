...
根据后端接口文档，我已经完成了前端代码的修改，实现了完整的接口调用和权限控制。以下是具体的修改内容：

## 1. 添加用户角色检测和权限控制
- 添加了 currentUserRole() 函数获取当前用户角色
- 添加了 isAdmin() 函数检查是否为管理员
- 添加了 isMerchantOwner() 函数检查是否为商家自己
## 2. 实现完整的接口调用
### 已实现的接口：
- 申请商家 ： applyMerchant
- 查询商家 ： selectMerchant
- 获取商家详情 ： selectMerchantById/{id}
- 更新商家 ： updateMerchant
- 删除商家 ： deleteMerchant/{id}
- 查询申请进度 ： queryApplyProgress
- 根据状态查询商家 ： selectMerchantByStatus
- 模糊搜索商家 ： searchMerchant
### 权限控制实现：
- 管理员 ：只能修改商家的审核状态，不能修改其他信息
- 商家 ：只能修改自己的基本信息，不能修改审核状态
- 权限检查 ：在审核、删除等操作前检查用户权限
### 技术优化：
- 所有接口调用都正确传递 userId 作为查询参数
- 添加了完整的错误处理和用户反馈
- 实现了权限控制的前端验证
- 确保了数据的安全性和一致性
现在，商家管理页面已经完全符合需求要求，能够正确调用后端接口并根据用户角色限制操作权限。